...
foreach ($slides as $filename => $item) {
	...
	$slidesHtml .= <<<HTML
        ...
        \n    <img src="http://dasg.ac.uk/images/slides/{$filename}.jpg" alt="DASG Fieldwork Image"/>
        \n</a>
            ...
            <div class="searchColumn">
                <div id="fieldworkSearchBox">
                    <form action="">
                        ...
                        <!--label for="headOnly">Headwords Only:</label>
                        <input type="checkbox" id="headOnly" name="headOnly" {$headOnlyHtml}/-->
                        <br/>
...
if (!empty($enteredQuery)) {
	...
	$query = str_replace("'", "’", $query);			//replace any single quote with smart quote (as used in XML source docs)
	...
	$queryRegExp = "({$query}’|[^" . ACCENT_CHARSET . "]{$query}[^" . ACCENT_CHARSET . "])|(^{$query}$)|(^{$query}[^" . ACCENT_CHARSET . "])|([^" . ACCENT_CHARSET . "]{$query}$)";
	...
	foreach (glob(ROOT . "/xml/archive/*.xml") as $filepath) {
	...
	//	uasort($results, "compare_count");
		...
		foreach ($results as $result) {
			...
			$description = $_GET["headOnly"] ? "" : $description = preg_replace("/({$queryRegExp})/iu", '<span class="hi">' . "$1" . '</span>', $result["description"]);
			$resultId = "result{$i}";
			$filename = str_replace(".xml", "", $result["filename"]);
			$encodedItem = base64_encode($filename . "|{$headword}|{$result["id"]}||{$enteredQuery}|r{$i}|{$_GET["l"]}|{$_GET["as"]}|");
			$geogHtml = "";
