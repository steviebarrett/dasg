...
$publicActions = ['search', 'searchText', 'getText', 'getPageList', 'getLineList', 'getLine'];

// Public visitors may only use read operations. Every write operation still
// requires the existing administrator login/session.
...
switch ($action) {
    case "search":
        ...
        $sql = "SELECT l.id AS id, page_id, l.number AS number, p.number AS pageNum, text_id,
                   diplomatic, vernacular, classical, translation, notes, team_comments
            FROM bdl_line l
    ...
    case "searchText":
        ...
        $like = '%' . $qt . '%';
        $sql = "SELECT id, author, firstline_c
                FROM bdl_text
                WHERE :empty = ''
                   OR CAST(id AS CHAR) LIKE :q
                   OR COALESCE(author, '') LIKE :q
                   OR COALESCE(firstline_d, '') LIKE :q
                   OR COALESCE(firstline_v, '') LIKE :q
                   OR COALESCE(firstline_c, '') LIKE :q
                   OR COALESCE(firstline_t, '') LIKE :q
                   OR COALESCE(edition, '') LIKE :q
                   OR COALESCE(notes, '') LIKE :q
                ORDER BY CAST(id AS UNSIGNED), id";
        $sth = $dbh->prepare($sql);
        $sth->execute([':empty' => $qt, ':q' => $like]);
